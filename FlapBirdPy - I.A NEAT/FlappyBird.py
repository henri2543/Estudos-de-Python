...
class Passaro:
    ...
    def mover(self):
        ...
        if deslocamento > 0 or self.y > (self.altura + 50):
            if self.angulo > self.ROTACAO_MINIMA:
                self.angulo = self.ROTACAO_MINIMA
        else:
            if self.angulo > - 90:
    ...
    def desenhar(self, tela):
        ...
        # se o pássaro estiver caindo o pássaro nao vai bater asa
        if self.angulo <= -80:
            self.imagem = self.IMGS[1]
            self.contagem_imagem = self.TEMPO_ANIMACAO * 2

        # desenhar a imagem
        imagem_rotacionando = pygame.transform.rotate(self.imagem, self.angulo)
        posicao_centro_imagem = self.imagem.get_rect(topleft=(self.x, self.y)).center
        retangulo = imagem_rotacionando.get_rect(center=posicao_centro_imagem)
        tela.blit(imagem_rotacionando, retangulo.topleft)
...
class Cano:
    ...
    def definir_altura(self):
        ...
        self.pos_base = self.altura + random.randrange(100, 450)
